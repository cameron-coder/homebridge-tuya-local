...
class TuyaDiscovery extends EventEmitter {
    ...
    _onDiscover(data) {
        if (this.discovered.has(data.gwId)) {
            // Check if IP changed
            if (this.discovered.get(data.gwId) !== data.ip) {
                this.log.info(`Discovery - Device ${data.gwId} moved from ${this.discovered.get(data.gwId)} to ${data.ip}.`);
                this.discovered.set(data.gwId, data.ip);
